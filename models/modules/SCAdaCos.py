...
if __name__ == "__main__":
    import sys
    sys.path.append("/DKUdata/2020/zhangyc/workspace/ASD/wilkinghoff_dc23")
    from datasets import dcase2023_t2
    from models.modules.two_branch_cnn import emb_model
    dataset = dcase2023_t2.dcase23_t2_dataset()
    encoder = emb_model(spectrogram_size=(1, 513, 313), spectrum_size=(1, 80000))
    classifier = AdaCos(256, 150)
    dataloader = DataLoader(dataset, batch_size=5, shuffle=True)
    for i, batch in enumerate(dataloader):
        _, spectrogram, spectrum, label, _, mixup, mixup_coeff = batch.values()
        emb = encoder({'spectrum':spectrum, 'spectrogram':spectrogram})
        output = classifier(emb['combo_emb'], mixup, label)
        print(output, label, mixup)
        break
