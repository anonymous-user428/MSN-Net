...
# directory path
# GROUND_TRUTH_DATA_DIR = f"ground_truth_data"
# GROUND_TRUTH_DOMAIN_DIR = f"ground_truth_domain"
# GROUND_TRUTH_ATTRIBUTES_DIR = f"ground_truth_attributes"
...
def output_result(task, target_dir, machines, section_ids, result_dir, additional_result_dir, seed, tag, out_all=False):
    print(target_dir)
    csv_lines = []
    ...
    for machine_idx, target_machine in enumerate(machines):
        ...
        for section_id in section_ids:
            ...
            for domain in ['source', 'target']:
                if 'dcase2020' in task and domain == 'target': continue
        ...
        performance = numpy.nan_to_num(performance, nan=0)
        ...
        for dataset_type in ["source", "target"]:
            if dataset_type == 'target': continue
    ...
    performance_over_all = numpy.nan_to_num(performance_over_all, nan=0)
    ...
    all_perf = numpy.nan_to_num(all_perf, nan=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, default='dcase2023')

    args = parser.parse_args()

    GROUND_TRUTH_DATA_DIR = f"{args.task}_ground_truth/ground_truth_data"
    GROUND_TRUTH_DOMAIN_DIR = f"{args.task}_ground_truth/ground_truth_domain"
    GROUND_TRUTH_ATTRIBUTES_DIR = f"{args.task}_ground_truth/ground_truth_attributes"

    if 'dcase2020' in args.task:
        OFFICIAL_SCORE_COLUMNS = [
            "official score", "arithmetic mean", "harmonic mean (source)", "harmonic mean (target)",
            "ToyCar AUC (source)", "ToyConveyor AUC (source)", "fan AUC (source)", "slider AUC (source)", "pump AUC (source)", "valve AUC (source)",
            "Toycar AUC (target)", "ToyConveyor AUC (target)", "fan AUC (target)", "slider AUC (target)", "pump AUC (target)", "valve AUC (target)",
        ]
        SCORE_COLUMNS = [
            "official score", "arithmetic mean", "harmonic mean (source)", "harmonic mean (target)", 
            "ToyCar Score (source)", "ToyConveyor Score (source)", "fan Score (source)", "slider Score (source)", "pump Score (source)", "valve Score (source)", 
            "ToyCar Score (target)", "ToyConveyor Score (target)", "fan Score (target)", "slider Score (target)", "pump Score (target)", "valve Score (target)", 
        ]
        PAPER_OFFICIAL_SCORE_COLUMNS = ["h-mean", "a-mean", "fan", "pump", "slider", "ToyCar", "ToyConveyor", "valve"]
        SYSTEM_OFFICIAL_SCORE_INDEXES = ["AUC (source)", "AUC (target)", "pAUC (source, target)", "TOTAL score"]
    elif 'dcase2021' in args.task:
        OFFICIAL_SCORE_COLUMNS = [
            "official score", "arithmetic mean", "harmonic mean (source)", "harmonic mean (target)",
            "ToyCar AUC (source)", "ToyTrain AUC (source)", "fan AUC (source)", "slider AUC (source)", "gearbox AUC (source)", "pump AUC (source)", "valve AUC (source)",
            "Toycar AUC (target)", "ToyTrain AUC (target)", "fan AUC (target)", "slider AUC (target)", "gearbox AUC (target)", "pump AUC (target)", "valve AUC (target)",
        ]
        SCORE_COLUMNS = [
            "official score", "arithmetic mean", "harmonic mean (source)", "harmonic mean (target)", 
            "ToyCar Score (source)", "ToyTrain Score (source)", "fan Score (source)", "slider Score (source)", "gearbox Score (source)", "pump Score (source)", "valve Score (source)", 
            "ToyCar Score (target)", "ToyTrain Score (target)", "fan Score (target)", "slider Score (target)", "gearbox Score (target)", "pump Score (target)", "valve Score (target)", 
        ]
        PAPER_OFFICIAL_SCORE_COLUMNS = ["h-mean", "a-mean", "ToyCar", "ToyTrain", "fan", "gearbox", "pump", "slider", "valve"]
        SYSTEM_OFFICIAL_SCORE_INDEXES = ["AUC (source)", "AUC (target)", "pAUC (source, target)", "TOTAL score"]
    elif 'dcase2022' in args.task:
        OFFICIAL_SCORE_COLUMNS = [
            "official score", "arithmetic mean", "harmonic mean (source)", "harmonic mean (target)",
            "ToyCar AUC (source)", "ToyTrain AUC (source)", "fan AUC (source)", "slider AUC (source)", "gearbox AUC (source)", "bearing AUC (source)", "valve AUC (source)", 
            "Toycar AUC (target)", "ToyTrain AUC (target)", "fan AUC (target)", "slider AUC (target)", "gearbox AUC (target)", "bearing AUC (target)", "valve AUC (target)",
        ]
        SCORE_COLUMNS = [
            "official score", "arithmetic mean", "harmonic mean (source)", "harmonic mean (target)", 
            "ToyCar Score (source)", "ToyTrain Score (source)", "fan Score (source)", "slider Score (source)", "gearbox Score (source)", "bearing Score (source)", "valve Score (source)",  
            "ToyCar Score (target)", "ToyTrain Score (target)", "fan Score (target)", "slider Score (target)", "gearbox Score (target)", "bearing Score (target)", "valve Score (target)",
        ]
        PAPER_OFFICIAL_SCORE_COLUMNS = ["h-mean", "a-mean", "ToyCar", "ToyTrain", "fan", "gearbox", "bearing", "slider", "valve"]
        SYSTEM_OFFICIAL_SCORE_INDEXES = ["AUC (source)", "AUC (target)", "pAUC (source, target)", "TOTAL score"]
    elif 'dcase2023' in args.task:
        OFFICIAL_SCORE_COLUMNS = [
            "official score", "arithmetic mean", "harmonic mean (source)", "harmonic mean (target)",
            "ToyCar AUC (source)", "ToyTrain AUC (source)", "fan AUC (source)", "slider AUC (source)", "gearbox AUC (source)", "valve AUC (source)", "bearing AUC (source)",
            "Toycar AUC (target)", "ToyTrain AUC (target)", "fan AUC (target)", "slider AUC (target)", "gearbox AUC (target)", "valve AUC (target)", "bearing AUC (target)",
        ...
        SCORE_COLUMNS = [
            "official score", "arithmetic mean", "harmonic mean (source)", "harmonic mean (target)", 
            "ToyCar Score (source)", "ToyTrain Score (source)", "fan Score (source)", "slider Score (source)", "gearbox Score (source)", "valve Score (source)", "bearing Score (source)",
            "ToyCar Score (target)", "ToyTrain Score (target)", "fan Score (target)", "slider Score (target)", "gearbox Score (target)", "valve Score (target)", "bearing Score (target)",
        ...
        PAPER_OFFICIAL_SCORE_COLUMNS = ["h-mean", "a-mean", "bearing", "fan", "gearbox", "slider", "ToyCar", "ToyTrain", "valve", \
    ...
    team_dirs = glob.glob(teams_root_dir+"/"+"*_"+args.task.split("_")[-1]) if ("dcase2020" in args.task) or ("dcase2021" in args.task) or ("dcase2022" in args.task) else \
                list(numpy.sort(glob.glob(teams_root_dir + "/*" * args.dir_depth)))
